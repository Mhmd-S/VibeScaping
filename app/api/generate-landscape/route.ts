...
export async function POST(request: NextRequest) {
  try {
    const { imageBase64, mimeType, isRevision, revisionNotes } =
      await request.json();
    ...
    if (isRevision && revisionNotes && revisionNotes.length > 0) {
      ...
      const annotationsList = revisionNotes
        ...
        .join("\n");
      prompt = `This is a landscape architecture map that needs revisions. The user has marked areas with annotations in red indicating what changes they want. Please revise the landscape map according to these requested changes:

        ${annotationsList}

        Generate a new version of this landscape architecture map incorporating all the requested changes. Maintain the same overall style and quality, but apply the specific modifications indicated. Do not include any text or annotations in the output image.`;
    } else {
      // Initial generation prompt
      prompt =
        "Here is my property, create a professional landscape architecture style map. It should be a perfect translation of the property in the image. Do not add any additional elements or text. Do not include shadows.";
    }
    ...
    // Construct multimodal content with image and text
