...
const REFERENCE_FILES = ['refrence1.png', 'refrence2.png'];

async function loadReferenceImages() {
    const referencesDir = path.join(process.cwd(), 'public', 'refrences');
    ...
    try {
        ...
        console.warn('Failed to load reference images; proceeding without them.', error?.message);
        return [];
    }
}
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (isRevision && revisionNotes && revisionNotes.length > 0) {
            ...
            prompt =
                'Here is my property. Create a professional landscape architecture style map that is a faithful replica of the provided image. Use the attached reference landscape style images to guide the aesthetic, materials, and layout quality. Do not include any text or annotations in the output image.';
        }
        ...
        try {
            ...
            console.warn('Image generation model failed, trying alternative model:', expError.message);
            ...
            try {
                ...
                console.warn('Alternative model failed, trying stable model:', altError.message);
